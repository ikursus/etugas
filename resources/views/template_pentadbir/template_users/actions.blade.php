<a href="{{ route('pentadbir.users.edit', $item->id) }}" class="btn btn-info btn-sm">
    EDIT
</a>
...
<div class="modal fade" id="modal-delete-{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        ...
        <form method="POST" action="{{ route('pentadbir.users.destroy', $item->id) }}">
            @csrf
            @method('DELETE')
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        Adakah anda pasti ingin menghapuskan rekod {{ $item->name }}?
                    </div>
